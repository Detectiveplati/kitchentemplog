...
function renderActiveCooks() {
  ...
  cooks.forEach(cook => {
    ...
    card.innerHTML = `
      ...
      ${cook.endTime ? `
        <div class="info-row">
          <input type="number" step="0.1" min="0" max="150" placeholder="Core °C" value="${cook.temp}"
            onchange="updateTemp(${cook.id}, this.value)">
          <input type="number" min="1" step="1" placeholder="Trays" value="${cook.trays}"
            onchange="updateTrays(${cook.id}, this.value)">
